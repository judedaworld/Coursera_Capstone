...
spacex_df = pd.read_csv("spacex_launch_dash.csv", nrows=1000)
...
def get_pie_chart(entered_site):
    filtered_df = spacex_df.drop(labels=['Unnamed: 0'], axis=1)
    if entered_site == 'ALL':
        filtered_df = filtered_df[filtered_df['class']==1]
        filtered_df = (filtered_df.groupby(['Launch Site']).count())/len(filtered_df)
        filtered_df.reset_index(inplace=True)
        filtered_df = filtered_df.iloc[:, 0:2]
        filtered_df.rename(columns={'Flight Number': 'percent'}, inplace=True)
        ...
        filtered_df = (filtered_df.groupby(['class']).count())/len(filtered_df)
        filtered_df.reset_index(inplace=True)
        filtered_df = filtered_df.iloc[:, 0:2]
        filtered_df.rename(columns={'Flight Number': 'percent'}, inplace=True)
        fig = px.pie(filtered_df, values='percent', 
